    ...
    if args.len() >= 3 {
        ...
        if args[2] == "--help" {
            ...
            match args[1].as_str() {
                ...
                _ => eprintln!("Error: '--filter' cannot be used on this argument '{}'", args[1]),
            }
        } else if args[2] == "--defilter" {
            match args[1].as_str() {
                "--ma" => ma_filter.do_defilter(),
                _ => eprintln!("Error: '--defilter' cannot be used on this argument '{}'", args[1]),
            }
        }
    }
...
// EXECUTE
    ...
    match args[1].as_str() {
        ...
        "--ma" => {
            ...
            if args.len() == 2 || (args.len() >= 3 && args[2] != "--help" && args[2] != "--filter" && args[2] != "--defilter") {
