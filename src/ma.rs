...
pub fn parse_file_and_delete_attribute(
    ...
    let available_attributes = vec![ // Array which holds all available attributes
        "connections",
        "object_events",
        "warp_events",
        "bg_events",
    ...
    // ALGORITHM
        for entry in fs::read_dir(dir)? {
            ...
            } /* else&& selected_attributes[0] != String::from("None")let mut map = serde_json::from_str(&text).expect("Error: Failed to read file");
    ...
    println!("  Maps successfully modified: {}", change_count / selected_attributes.len() as i16); // The devision is there because every deletion per map, counts the counter so for 3 attributes, the final count is 3 times as high
    println!("  Map-Attributes:");
    for ma in selected_attributes.iter() {
        println!("      {}", ma);
    }

    if failed_maps.len() >= 1 && selected_attributes[0] != String::from("None") { // If there is any failure and theres at least one available argument, do that
        println!("  Maps failed to read: {}, Path(s):", failed_count);
        for maps in failed_maps { // Prints path of every map failed to read because 'map.json' is not available
